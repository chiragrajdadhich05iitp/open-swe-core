...
def test_reviewer_thread_id() -> None:
    assert (
        reviewer_thread_id("acme", "repo", 7) == "0e0feb04-05d3-5925-969e-1051f3741622"
    )


def test_pr_comment_thread_id() -> None:
    assert (
        pr_comment_thread_id("acme", "repo", 7)
        == "57ba1274-081d-5cad-a53e-d5f7610f2191"
    )


def test_review_style_thread_id() -> None:
    assert (
        review_style_thread_id("acme", "repo") == "7bdeb197-4322-52f1-a815-b485aeae5fdb"
    )
...
def test_slack_thread_id_without_nonce() -> None:
    assert (
        slack_thread_id("C123", "1700000000.000100")
        == "e624d56b-70ab-5512-b966-215798b600a2"
    )
...
def test_baby_sit_lock_thread_id() -> None:
    assert (
        baby_sit_lock_thread_id("acme/repo#7") == "c81483d2-06cc-51e1-a06d-1d466d3fe3a1"
    )
