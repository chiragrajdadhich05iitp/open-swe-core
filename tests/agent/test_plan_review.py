...
def test_plan_approved_slack_text_mentions_comments_and_actor() -> None:
    from agent.dashboard.plan_api import (
        _plan_approved_slack_blocks,
        _plan_approved_slack_text,
    )
...
async def test_list_plan_comments_raises_with_flag(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agent.dashboard import plan_store
...
async def test_clear_plan_comments_deletes_each(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agent.dashboard import plan_store
...
async def test_save_plan_reads_html_file_from_sandbox(
    ...
    assert reads == [
        (
            "/workspace/plans/2026-06-29-test-plan.html",
            0,
            save_plan_tool._MAX_PLAN_LINES,
        )
    ]
    assert saved == {
        "thread_id": "thread-1",
...
async def test_list_workflow_approvals_returns_records(
    ...
    assert result["approvals"][0]["diffStats"] == {
        "files": 1,
        "additions": 1,
        "deletions": 0,
    }
...
async def test_set_plan_status_preserves_plan_file_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agent.dashboard import plan_store
...
async def test_set_plan_status_records_approver_audit(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agent.dashboard import plan_store
...
async def test_save_plan_content_clear_comments_flag(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agent.dashboard import plan_store
...
async def test_get_plan_returns_approval_attribution(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_update_plan_saves_and_mirrors_sandbox(
    ...
    result = await plan_api.update_plan(
        "t1",
        plan_api.PlanUpdate(html="# New\n\ndo x"),
        session={"sub": "a", "email": None},
    )
...
async def test_update_plan_blocked_once_approved(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from fastapi import HTTPException
...
async def test_approve_plan_hides_unreadable_thread(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from fastapi import HTTPException
...
async def test_concurrent_plan_approvals_dispatch_once(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import asyncio
...
async def test_reject_plan_can_mark_revising_without_dispatch(
    ...
    result = await plan_api.reject_plan(
        "t1",
        plan_api.PlanRejection(dispatch=False),
        session={"sub": "a", "email": None},
    )
