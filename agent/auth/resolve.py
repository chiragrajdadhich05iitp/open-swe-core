...
async def get_github_token_for_user(ls_user_id: str, tenant_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            "GitHub auth API HTTP error: %s - %s",
...
async def leave_failure_comment(
    ...
    if source in ("github", "github_push"):
        logger.warning(
            "Auth failure for GitHub-triggered run (no token to post comment): %s",
            message,
        )
        return
...
async def _resolve_bot_installation_token(thread_id: str) -> tuple[str, str | None]:
    ...
    logger.info(
        "Using GitHub App installation token for thread %s (bot-token-only mode)",
        thread_id,
    )
    return _cache_resolved_github_token(
...
async def resolve_github_token(
    ...
    if not source:
        logger.error(
            "Missing source for thread %s; cannot route auth failure responses",
            thread_id,
        )
